...
public class WordLib {
    ...
    public static void showWord(Strip strip, int wordIdx) {
        showWord(strip, wordIdx, 0xffffff);
    }

    public static void showWord(Strip strip, int wordIdx, int color) {
    ...
    public static boolean easeWord(Strip strip, int wordIdx, int color) {
        int[] word = words[wordIdx];
        for(int i=0; i<word.length; i++) {
            if(!strip.easePixelColor(word[i], color)) {
                return false;
            }
        }
        return true;
